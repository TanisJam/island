 ...
 * and `terrains` (Slice 3b) are populated so far; the remaining 2
 * collections (`world-objects`, `items`) are added by later slices (4, 5)
 * as their descriptors land — a collection with a `COLLECTIONS` registry
 * entry but no descriptor here is not yet selectable through the generic
 * engine.
 */
