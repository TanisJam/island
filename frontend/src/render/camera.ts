...
/**
 * Centers the camera on the player's (possibly mid-tween) `renderPos`. Falls
 * back to the zone's geometric center when no player entity is present in
 * the frame (e.g. before the first sync). `+0.5` targets the middle of the
 * player's current tile-space position, matching how tiles are drawn at
 * `tile * PX` with size `PX` in `render/canvas.ts`.
 */
export function cameraOffset(frame: Frame, viewport: Viewport): CameraOffset {
  ...
  return {
    ox: viewport.width / 2 - camX * PX,
    oy: viewport.height / 2 - camY * PX,
  };
}
